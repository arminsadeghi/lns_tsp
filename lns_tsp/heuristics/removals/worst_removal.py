...
def worst_removal(instance: TSPInstance, tour: list, num_to_remove: int) -> list:
    """Remove a random subset of nodes from the tour."""
    temp_tour = solver_core.worst_removal(instance, tour, len(tour), num_to_remove)
    new_tour = [x for x in temp_tour if x != -1]
    deleted_nodes = [x for x in range(len(tour)) if x not in new_tour]
